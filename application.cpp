    ...
    menu_{}
{
    ...
    render_ = std::make_unique<Render>(width, height);
    ...
    map_ = std::make_unique<Map>(Render::LoadTexture("textures/Map.rgba", 1024, 1024), 0.0f, 0.0f, 620.0f, 620.0f);

    render_->AddUnitTexture(Render::LoadTexture("textures/Bug_1.rgba", 64, 64), UnitType::Bug1, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Bug_2.rgba", 64, 64), UnitType::Bug2, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Bug_3.rgba", 64, 64), UnitType::Bug3, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Gun.rgba", 64, 64), UnitType::Gun, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Laser.rgba", 64, 64), UnitType::Laser, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/RocketGun.rgba", 64, 64), UnitType::RocketGun, 40.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Rocket.rgba", 64, 64), UnitType::Rocket, 20.0f, 40.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Shot.rgba", 64, 64), UnitType::Shot, 5.0f, 5.0f);
    render_->AddUnitTexture(Render::LoadTexture("textures/Boom.rgba", 128, 128), UnitType::Boom, 1.0f, 1.0f);
}
...
void Application::Run()
{
    ...
    while (!glfwWindowShouldClose(window_))
    {
        ...
        {
            Color color;
            if(map_->CanPut({xpos_, ypos_}, {40.0f, 40.0f}))
...
void Application::MouseEvent(GLFWwindow* window, int button, int action, int mode)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
        ...
        if(0 < xpos && xpos < 620 && 0 < ypos && ypos < 620)
        {
            ...
            if(selected == 1)
            {
                if(app->map_->gold_ >= 100)
                {
                    app->map_->gold_ -= 100;
                    app->map_->Put(UnitType::Gun, xpos_, ypos_);
                }
            }
            if(selected == 2)
            {
                if(app->map_->gold_ >= 200)
                {
                    app->map_->gold_ -= 200;
                    app->map_->Put(UnitType::Laser, xpos_, ypos_);
                }
            }
            if(selected == 3)
            {
                if(app->map_->gold_ >= 300)
                {
                    app->map_->gold_ -= 300;
                    app->map_->Put(UnitType::RocketGun, xpos_, ypos_);
                }
            }
        }
    }
...
{
    map_ = std::make_unique<Map>(map_->texture_id_, 0.0f, 0.0f, 620.0f, 620.0f);
}
