...
Button::Button(GLuint texture_id, glm::vec2 position, glm::vec2 size) :
...
void Button::Render(class Render& render) const
{
    ...
    auto body_size = size_ - glm::vec2{4.0f, 4.0f};
...
glm::vec4 Button::BorderColor() const noexcept
{
    if(hovered_)
...
{
    buttons_.emplace_back(texture_id, glm::vec2{x, y}, glm::vec2{w, h});
}
